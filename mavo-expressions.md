...
**IN A NUTSHELL:** Expressions let you present the current value of properties in other elements and perform simple computations.
...
If your HTML text contains brackets whose content you don’t want to be an expression, you can change the syntax with the `mv-expressions` attribute. For example, to switch to a double curly brace syntax (common in other systems), you can use `mv-expressions="{{ }}"`. To disable expressions in an element and its descendants altogether, use `mv-expressions="none"`.
...
**Slider value: 50/100**
<input type="range" style="width:50%" value="50">
...
Functions transform values into different values, for example they can do math on a number, or tell if your text contains some other text. They are written with their name, and then a comma-separated list of parameters in parentheses, e.g. `min(2, 5)`.

[Learn more about All MavoScript functions](https://mavo.io/docs/functions)

### Operators

Operators are a shorter and more readable way to write common functions. For example, imagine if every time we wanted to subtract two numbers we had to write e.g. `subtract(2, 3)`. We would end up writing lots of parentheses for even simple expressions and it would be hard to understand what the expression actually does. Instead, we can write `2 - 3` instead, which is much shorter and more readable.

Mavo supports all common math and logical operators, as well as some more like `where`.

[Learn more about All MavoScript operators](https://mavo.io/docs/operators)
...
[Learn more about All MavoScript special properties](https://mavo.io/docs/special-properties)
...
- **Lists of values**, just like those you get by referring a list property from outside the list, can be written as `list(value1, value2, value3, ...)` You can read more about `list()` [here](https://mavo.io/docs/functions#list-items).
...
<div style="background-color: hsl(180, 50%, 50%); padding: 1em; border: 1px solid #ccc;">
	<input type="range" style="width: 100%;" value="180" max="360">
	<input type="range" style="width: 100%;" value="50">
	<input type="range" style="width: 100%;" value="50">
	<input readonly="" value="hsl(180, 50%, 50%)" style="width: 100%; margin-top: 1em;">
</div>
...
References and simple arithmetic are useful, but not always enough. For this purpose, Mavo defines a number of [useful functions](https://mavo.io/docs/functions) you can use inside expressions, such as: `pow(base, exponent)`, `round(number, decimals)`, `sqrt(number)` and many more.
...
= [pow(base, exponent)]``` 2<sup>3</sup> = 8 **Note:** If what you're printing
contains characters other than letters, numbers and underscores, you will need
to use quotes around it, just like in CSS `font-family` or HTML attributes. ##
Expressions and Groups Groups are properties that contain other properties, or
that explicitly have a `typeof` attribute (also called "complex properties", and
they are analogous to objects in programming languages). If there are groups and
there are properties with the same name inside and outside the group, the value
**closest** to your expression will be used, with child (inner) groups having
priority over parent (outer) groups. #### Example ```html
...
Using a property with multiple values (i.e. referring to a property inside a list from outside the list) in an expression is not very useful on its own. However, there are functions that help you use such lists of values in interesting ways, such as `count(property)`, `sum(property)`, `average(property)` and [more](https://mavo.io/docs/functions).
...
<div mv-app="catbook" mv-storage="#data">
  ...
  <ul>
    <li property="cat" mv-list-item>
      <span property="name">Vector</span>
      <span property="age">10</span>
      <!-- A "not" that expression inside list item refers to the value in the current list item -->
    </li>
    <li property="cat" mv-list-item>
      <span property="name">Lea</span>
  ...
  <ul>
    ...
    <li>Number of cats: [count(age)]</li>
    <li>Average cat age: [average(age)] also works --> [avg(age)]</li>
    <li>Sum of all ages (why? because we can): [sum(age)]</li>
    <!-- caps don't matter -->
...
- All cat ages: 12, 14, 6, 2.5, 8, 8
- Number of cats: 6
- Average cat age: 8.416666666666666
- Sum of all ages (why? because we can): 50.5
- Number of cats over 5 years old: 5

**Note:** that if your expression is inside a list item, any properties inside that item will refer to their **current value**, whereas any properties outside the list will have multiple values. If your expression is outside the list you will get **all values**.

You can also do math or comparisons with lists. For example, `[sum(age + 5)]` will add 5 to every age and then sum the result. You can use either symbols (`+`, `*`, `>`, `==`) etc to perform these operations, or their function counterparts.
...
The `if()` function is very powerful, but can be confusing when all you want is to toggle content. For this, you can also use the `mv-if` attribute to hide or show content based on the value of an expression. `mv-if` does not require braces or other syntax. Besides readability, `mv-if` has another benefit: elements hidden with it are completely removed from the page, not just hidden.
...
<div
  mv-app="class-toggle"
  class="[if(rating < 3, bad, if(rating > 3, good, ok))]"
>
  <label
    >Rating:
...
```html
<p>Slider value: <span mv-value="strength">50</span>/100</p>
...
<div mv-list mv-mode="edit">
  <span property="list1" mv-list-item>Add list1</span>
</div>
<div mv-list mv-value="list1">
  <span property="list2" mv-list-item></span>
</div>
```
...
Mavo provides a way for attributes to contain expressions without triggering browser errors or false loading indicators. Suppose we have `<img src="[foo]">`. Till the expression is evaluated, this causes a browser error since `[foo]` is not a valid URL. With the `mv-attr-src`, we can avoid this error, like so: `<img mv-attr-src="foo" src="fallback_URL">`. `mv-attr-src="foo"` becomes `src="[foo]"`. When `foo` is evaluated, its value will replace `fallback_URL`, and a new image will be loaded.
...
<meta
  property="url"
  content="https://designftw.mit.edu/lectures/html/img/adamcatlace.jpg"
/>
<img
  property="image"
...
<img src="https://designftw.mit.edu/lectures/html/img/adamcatlace.jpg">

**Image URL:** https://designftw.mit.edu/lectures/html/img/adamcatlace.jpg

**Note:** The value of `mv-attr-*` is an expression, and it doesn't require braces (or whatever other syntax is defined via `mv-expressions`) around it. If you include them, they will be considered part of your expression.

## ADVANCED MavoScript vs JavaScript

This section requires an understanding of JavaScript. It is aimed at advanced users and plugin developers who need to use JavaScript to use Mavo.

MavoScript is based on a subset of JavaScript, with a few simplifications to make it easy to use even by people who do not need to understand JavaScript.

For those who know JavaScript, these are a few key differences:

| JavaScript                                                                 | MavoScript                                                                                                                              |
| :------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------- |
| Math and logical operations with arrays produce nonsensical results        | Array math & logical operations works element-wise. This is why things like `count(rating > 3)` work in MavoScript, but wouldn't in JS. |
...
| Things like `3 > 2 > 1` return unexpected results (false in this case)     | `3 > 2 > 1` has the same result as in math (`true`)                                                                                     |
| `foo.bar.baz` will throw an error if either `foo` or `foo.bar` don't exist | Every operator can have multiple operands, including `.` If either `foo` or `foo.bar` don't exist, `foo.bar.baz` will just return `""`. |
...
```html
<div mv-app="function-calls">
  <input
    type="url"
    property="url"
    value="https://mavo.io/docs/storage#url-params"
  />
  <output>[encodeURIComponent(url)]</output>
</div>
```

<https://mavo.io/docs/storage#url-params>
